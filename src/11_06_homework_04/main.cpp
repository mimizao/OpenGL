...
GLFWwindow *init(int &initResult, int width, int height, const char *title, GLFWmonitor *monitor = nullptr, GLFWwindow *share = nullptr);
...
{
	int initResult = 0;
	auto window = init(initResult, SCR_WIDTH, SCR_HEIGHT, "11_06_homework_04");
	if (initResult != 0)
	{
		return initResult;
	}
	...
	//! position attribute
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 8 * FLOAT_SIZE, (void *) nullptr);
	...
	auto diffuseMap = loadTexture("../resources/textures/container2.png");
	auto specularMap = loadTexture("../resources/textures/new_specular.png");
	auto emissionMap = loadTexture("../resources/textures/emission.jpg");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//! emission params
		cubeShader.setFloat("emissionLight", 1.0f + static_cast<float>(sin(glfwGetTime())));
		cubeShader.setFloat("emissionMove", static_cast<float>(glfwGetTime()));
		...
		lampShader.setMat4("model", glm::scale(glm::translate(model, lampPos), glm::vec3(0.2f)));
...
GLFWwindow *init(int &initResult, int width, int height, const char *title, GLFWmonitor *monitor, GLFWwindow *share)
{
	glfwInit();
